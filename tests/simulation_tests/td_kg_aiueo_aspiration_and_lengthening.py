...
class TestOtmlWithTAspiration(SimulationTestCase):
    def setUp(self):
        ...
        self.unit_tests_log_file_name = "../../logging/{}_td_kg_aiueo_aspiration_and_lengthening_400_INF_INF_{}.txt".format(
            platform.node(), simulation_number)
        self._set_up_logging()
        configurations["CORPUS_DUPLICATION_FACTOR"] = 1
        self.feature_table = FeatureTable.load(
            get_feature_table_fixture("td_kg_aiueo_aspiration_and_lengthening_feature_table.json"))
        ...
        target_energy = self.get_target_hypothesis_energy(self.feature_table,
                                                          "td_kg_ai_aspiration_and_lengthening_target_constraint_set.json",
                                                          corpus,
